...
for isbn in vols_need_enrich:
    ...
    val = sp.find('div', {'class': 'pre-order-street-date'}).text \
        .replace('Release date:', '') \
            .replace('ESTIMATED TO SHIP', '') \
                .replace('Ship date is an estimate and not guaranteed', '') \
                    .replace('Pre-order FAQ', '') \
                        .strip()
    vol['release_date'] = str(datetime.strptime(val, '%m/%d/%Y').date())

    # print(vol['release_date'])
    # print()

    completed += 1
    progress = round((completed / len(vols_need_enrich)) * 50)
    remaining = 50 - progress
    percentage = round((completed / len(vols_need_enrich)) * 100, ndigits=2)

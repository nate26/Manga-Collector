...
export class UserService {
  ...
  readonly userIdFromRoute$ = this._activatedRoute.queryParams.pipe(
    ...
    switchMap(
      params => of({ data: { get_user: { user_id: '' } }, error: null })
      // this._apollo.query<{ get_user: { user_id: string } }>({
      //   query: gql`
  ...
  private readonly _userIdOnRoute = toSignal<string, string>(this.userIdFromRoute$, {
    initialValue: ''
  });
  ...
  login(
    ...
    return (
      of({ data: null, error: Error('no login') } as any)
        // this._apollo
        //   .query<{ login: UserData }>({
        //     query: gql`
  ...
  signUp(
    ...
    if (!email || !username || !password) {
      return throwError(() =>
        Error('Email, username, and password are required to sign up for an account.')
      );
    }
    return (
      of({ data: null, error: Error('no login') } as any)
        // this._apollo
        //   .query<{ sign_up: UserData }>({
        //     query: gql`

...
import { AuthenticationData, UserService } from '../data/user.service';
...
export function authInterceptor(req: HttpRequest<unknown>, next: HttpHandlerFn) {
    ...
    return iif(
        () => parseFloat(authentication.expiration) > (Date.now() / 1000),
        ...
        inject(HttpClient).post<AuthenticationData>(
            ...
            map(authentication => ({ ...userService.userData(), authentication })),

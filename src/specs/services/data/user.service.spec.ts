...
describe('UserService', () => {
  ...
  describe('User Data', () => {
    ...
    it('should check username', done => {
      ...
      forkJoin([service.checkUsername('test1'), service.checkUsername('test2')]).subscribe({
        next: data => {
  ...
  describe('Local Storage', () => {
    ...
    it('should login with email and password and save to local storage', () => {
      ...
      expect(httpMock.post).toHaveBeenCalledOnceWith('http://localhost:8050/login', {
        email: 'user@gmail.com',
        password: 'password'
      });
      expect(localStorage.getItem('username')).toBe('test');
    ...
    it('should fail to login without email or password', done => {
      forkJoin(
        ...
        ].map(obs => obs.pipe(catchError((err: Error) => of(err.message))))
      ).subscribe({
        next: data => {
          expect(data).toEqual(Array(4).fill('Email and password are required to login.'));
          done();
        },
        error: fail
      });
    ...
    it('should signup with email, username, and password and save to local storage', () => {
      ...
      expect(userData!).toEqual(MOCK_USER_DATA);
      expect(httpMock.post).toHaveBeenCalledOnceWith('http://localhost:8050/sign-up', {
        email: 'user@gmail.com',
        username: 'user',
        password: 'password'
      });
    ...
    it('should fail to signup without email, username, or password', done => {
      forkJoin(
        ...
        next: data => {
          expect(data).toEqual(
            Array(6).fill('Email, username, and password are required to sign up for an account.')
          );
          done();
        },
        error: fail
